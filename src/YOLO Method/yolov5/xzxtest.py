'''1.实时性问题
2.精检测的问题  粗检测前后两帧判断，精检测通过10帧以上进行判断,每10帧进行一次角点检测，在这10帧内，对每一个角点的逆行情况进行统计，超过一个阈值就进行警报
在窗口10帧期间，角点的数量是不会变化的
3.人头检测的精确度'''
...
def retrograde(old_x, old_y, new_x, new_y):  # 用来判断是否逆行的函数
    ...
    theta_ni_margin = 10  # 和逆行方向夹角在5°以内的规定为逆行
    ...
    for i, det in enumerate(pred):  # detections per image
        ...
        if len(det):
            ...
            if prospect_frame == 1 or (prospect_frame - 1) % gap == 0:
                ...
                if(prospect_frame)%gap ==0:
                    print(f'{((prospect_frame//gap)-1)*gap+1}-{(prospect_frame//gap)*gap}帧',Record)
                    '''将结果保存在txt中
                    with open(r'D:\Steven\yolov5\yolov5\runs\detect\reverse.txt', 'a') as f:##mode a ,不会删除原始数据
                        f.write(str(Record))
                        f.write("\n")  # 换行
                    '''
                #####接下来是精检测部分
