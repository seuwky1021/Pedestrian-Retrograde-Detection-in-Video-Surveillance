'''1.实时性问题
2.精检测的问题  30帧内 记录每一帧中所有角点的坐标，m帧i角点（x,y）坐标
前5帧先不考虑，第六帧开始，如果这一帧角点l有逆行现象发生，则判断l角点与5帧之前l角点的位置来判断是否真正逆行
3.人头检测的精确度'''
...
video_path = r'D:\qqfile_recv\20220825采集\奇奇怪怪的异常\醉汉摇头晃脑顺行.mp4'
#####yolo parameters
...
def retrograde(old_x, old_y, new_x, new_y):  # 用来判断是否逆行的函数
    ...
    ##对前后两帧的向量大小做要求,排除明显跟踪错误的点
    Vmin = 0
    Vmax = 1  # 有待修改
    if math.sqrt(Vx ** 2 + Vy ** 2) <= Vmin and math.sqrt(Vx ** 2 + Vy ** 2) > Vmax:
        print("有跟踪错误的点")
        return -1
    theta_margin = 60  # 顺行的余量为45°
    theta_ni_margin = 20  # 和逆行方向夹角在5°以内的规定为逆行
...
def rough_retro(old_x, old_y, new_x, new_y):
    Vx = new_x - old_x  # 横 向右为正
    Vy = new_y - old_y  # 纵 向下为正
    temp = [0, 0, Vx, Vy]  # 要检测的方向的向量，前两个数字代表起始点，后两个是向量的终点
    shunxing = [0, 0, 0, -1]  # 规定的顺行的方向，向右为x的正方向，向下为y的正方向，所以-1代表向上为顺行方向
    ...
    if math.sqrt(Vx ** 2 + Vy ** 2) <= Vmin and math.sqrt(Vx ** 2 + Vy ** 2) > Vmax:
        print("有跟踪错误的点")
    theta_margin = 60  # 顺行的余量为45°
    theta_ni_margin = 90  # 和逆行方向夹角在5°以内的规定为逆行
...
t0 = time.time()
global score  #一个根据yes和no的数目变化的分数 yes+1.0 no-0.5
score =45
    ...
    for i, det in enumerate(pred):  # detections per image
        ...
        if len(det):
            ...
            for xyxy in save_list:
                ...
                cv2.rectangle(area, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), 255,-1)  # 这里把头变大了



            if prospect_frame == 1 or (prospect_frame - 1) % gap == 0:
                p0 = cv2.goodFeaturesToTrack(im0_gray, mask=area, **feature_params)  # 角点检测
                RecordPts=np.zeros((gap,p0.shape[0],2))#  RecordPts[x,y].tolist()获取x帧y角点（m,n）坐标
                '''
                ...
                #print('here',len(good_old)==len(good_new))  #这里是为了证明在窗口10帧期间，角点的数量是不会变化的
                ...
                for i, (new, old) in enumerate(zip(good_new, good_old)):
                    ...
                    temp_result=retrograde(c, d, a, b)
                    if (prospect_frame-2)%gap ==0 :
                        RecordPts[(prospect_frame%gap)-2,i,0]=c#下标从0开始
                        RecordPts[(prospect_frame%gap)-2,i,1]=d
                        RecordPts[(prospect_frame % gap)-1, i, 0] = a
                        RecordPts[(prospect_frame % gap)-1, i, 1] = b
                    else:
                        RecordPts[(prospect_frame % gap)-1, i, 0] = a
                        RecordPts[(prospect_frame % gap)-1, i, 1] = b
                    ###精检测部分
                    if temp_result==1 and prospect_frame%gap>5:
                        Recheck = rough_retro(RecordPts[(prospect_frame % gap) - 6, i, 0],
                                             RecordPts[(prospect_frame % gap) - 6, i, 1],
                                             RecordPts[(prospect_frame % gap) - 1, i, 0],
                                             RecordPts[(prospect_frame % gap) - 1, i, 1]
                                             )

                        if Recheck == False:
                            print(f"第{prospect_frame}帧:最终判定:逆行")
                            retro_frame_list.append(prospect_frame)
                            #cv2.putText(im0, "Retrograde Or Not: YES", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 1,(0, 0, 255), 3)
                            if score < 70:
                                score += 1
                            flag = True
                        ...
                        #print(f"第{prospect_frame}帧:最终判定:无逆行")
                if flag == False:
                ...
                if score>50:
                    ...
                    cv2.putText(im0, 'Retrograde Or Not: NO', (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)
                cv2.putText(im0, f'score:{score}', (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 0, 255), 3)
