...
theta_ni_margin = 25  # 和逆行方向夹角在5°以内的规定为逆行
#2.0 优化了计算角度的函数

vc = cv2.VideoCapture(r"C:\Users\86137\Desktop\20220825采集\普通来回\来回5次.mp4")  # 读入视频文件
fourcc = cv2.VideoWriter_fourcc('I','4','2','0')
out = cv2.VideoWriter('12.mp4', fourcc, 30, (640,360))
...
while True:
    ...
    if prospect_frame == 1:  #第一帧图像虚化严重，放弃第一帧，从第二帧开始检测
        ...
        #判断逆行的阈值,根据ni+shun的值来综合改变
        if ni+shun<=10:
            threshhold=1
        elif 10<ni+shun<20:
            threshhold=2
        elif 20<=ni+shun<40:
            threshhold=int((ni+shun)/10)
        else:
            threshhold=4
        #综合前后5帧判断，实现精检测
        framenum=5
